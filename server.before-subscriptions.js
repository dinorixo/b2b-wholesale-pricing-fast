...
app.get("/api/dashboard", (req, res) => {
  ...
  res.json({
    ...
    summary: {
      ...
      rules: rules.length,
      unitsSold: units,
      totalDiscounts: Number(discounts.toFixed(2))
    }
  });
});
...
app.get("/api/customers", (req, res) => {
  ...
  orders.forEach(order => {
    const name =
      String(order.customerName || "Unknown customer").trim();
    ...
    if (!customers[name]) {
      customers[name] = {
        ...
        name,
        email: order.customerEmail || "",
        group: order.customerGroup || "Retail",
        orders: 0,
        revenue: 0,
        units: 0,
    ...
    customers[name].revenue += Number(order.total || 0);

    if (Array.isArray(order.items)) {
      order.items.forEach(item => {
        customers[name].units += Number(item.quantity || 0);
      });
    }

    if (
      order.createdAt &&
      (!customers[name].lastOrder ||
       order.createdAt > customers[name].lastOrder)
    ) {
      customers[name].lastOrder = order.createdAt;
    }
  });
...
app.get("/api/customers/:name", (req, res) => {
  ...
  const customerOrders = orders.filter(order =>
    String(order.customerName || "") === name
  );
...
app.get("/api/analytics/overview", (req, res) => {
  ...
  orders.forEach(order => {
    ...
    customerSales[customer] =
      (customerSales[customer] || 0) + total;

    groupSales[group] =
      (groupSales[group] || 0) + total;
    ...
    paymentSales[payment] =
      (paymentSales[payment] || 0) + 1;

    if (order.createdAt) {
      const date = String(order.createdAt).slice(0, 10);

      dailySales[date] =
        (dailySales[date] || 0) + total;
    }
    ...
    if (Array.isArray(order.items)) {
      order.items.forEach(item => {
        ...
        productSales[key].units += quantity;
...
app.put("/api/premium", (req, res) => {
  ...
  db.settings.premium = {
    ...
    plan: String(req.body.plan || "Free"),
...
app.get("/api/payments/summary", (req, res) => {
  ...
  orders.forEach(order => {
    ...
    result[status].orders += 1;
    result[status].revenue += Number(order.total || 0);
  });

  res.json({
    success: true,
    payments: Object.values(result)
      .map(item => ({
        ...item,
